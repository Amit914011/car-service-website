...
const UpdateData = () => {
 ...
 function handleChange(e){
  setFormData({...formData,[e.target.name]:e.target.vanl})
 }

 let {id} = useParams()
 async function getdata() {
    let result= await axios.get(`http://localhost:5000/api/viewData/${id}`)
 ...
 async function handleSubmit() {
    axios.put(`http://localhost:5000/api/updateData/${id}`)
