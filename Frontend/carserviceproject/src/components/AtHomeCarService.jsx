...
import image1 from '../assets/Car brand logo/card1.png';
import image2 from '../assets/Car brand logo/carservice2.png';
import image3 from '../assets/Car brand logo/caracservice.png';
import image4 from '../assets/Car brand logo/battery service.jpg';
...
import { Car, ChevronDown, Fuel, MoveLeft } from 'lucide-react';
...
export default function AtHomeCarService() {
  const [selectedCar, setSelectedCar] = useState(null); // To track the selected car details
  const [isOpen, setIsOpen] = useState(false); // To control modal visibility

  let cardDetails = [
    { id: 1, carName: "Car Service", carImage: image1 },
    { id: 2, carName: "Car Wash and Groom", carImage: image2 },
    { id: 3, carName: "Car AC Service", carImage: image3 },
    { id: 4, carName: "Battery Service", carImage: image4 },
  ];

  // Function to handle card click and open the modal
  const handleCardClick = (car) => {
    setSelectedCar(car);  // Set the selected car details
...
setFuel(false)
  ...
  return (
    <>
      <div className="mt-12 text-1xl md:text-3xl font-bold text-center py-5 bg-gray-100">
        <h1>At Home Car Service We Provide</h1>
      </div>

      <div className='w-full flex flex-wrap p-3 py-5 bg-gray-100'>
        <div className='w-full flex-wrap justify-center items-center flex gap-3 mb-3'>
          {cardDetails.map((car, index) => (
            <div
              key={index}
              className='w-[45%] md:w-[49%] border flex-col md:flex md:justify-between h-40 rounded-lg cursor-pointer hover:shadow-lg transition duration-300 ease-in-out flex justify-between'
              onClick={() => handleCardClick(car)} // Open modal on card click
            >
              <div className=' md:text-2xl font-bold pl-5 pt-3'>{car.carName}</div>
              <div className='md:flex md:items-end md:justify-end flex items-center justify-center'>
                <img src={car.carImage} alt={car.carName} className='rounded-lg w-[100px]' />
              </div>
            </div>
          ))}
        </div>
      ...
      {isOpen && selectedCar && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg overflow-hidden shadow-xl  w-[80%] h-[80%] p-12 relative">

      {main &&     <div className='bg-white rounded-lg overflow-auto   w-full h-full '>
        <h1 className=' text-xl md:text-3xl font-bold mb-2'>Experience The Best Car Service in <span className='uppercase'>Noida</span></h1>
        ...
        <div className='flex rounded-lg mb-2 w-full p-2 justify-between uppercase text-xl font-[600] border-2 border-gray-400 cursor-pointer' onClick={handleMain}>
       ...
       <input className='flex w-full rounded-lg p-2 justify-between uppercase text-xl font-[600] border-2 border-gray-400' type="number" placeholder='ENTER MOBILE NUMBER' required/>
       <button type='submit'  className='w-full mt-[10px] rounded hover:bg-red-800 text-center bg-red-600 p-2 text-xl font-[600] uppercase text-white'>check price for free</button>
       </form>
      </div>}
      {menu &&  <div className='bg-white rounded-lg overflow-auto   w-full h-full'>
    ...
    {model &&  <div className='bg-white rounded-lg overflow-auto   w-full h-full'>
    ...
    {fuel &&  <div className='bg-white rounded-lg overflow-auto   w-full h-full '>
        ...
        <div className='flex flex-wrap justify-evenly mt-[40px]'>
           {fuelType.map((p)=>(
            ...
            {/* <div className="flex justify-between items-center p-4 border-b">
              <h2 className="text-xl font-bold">{selectedCar.carName}</h2>
            ...
            <div className="p-4">
              <img src={selectedCar.carImage} alt={selectedCar.carName} className="w-full h-40 object-cover rounded-lg mb-4" />
              <p className="text-gray-600">Details about the {selectedCar.carName} service will go here.</p>
            </div> */}

            <div className="flex justify-end p-4 border-t absolute top-0 right-0">
              <button onClick={closeModal} title='Close' className="text-red-700 text-2xl px-4 py-2 rounded-lg">
              <CgCloseO />
              </button>
            </div>
          </div>
      ...
      {/* Example of one static card */}
      {/* <div className="shadow-lg rounded-lg p-2 h-24 bg-color1">
        <div className="overflow-hidden flex justify-between items-center"> */}
          {/* First Column: Heading and Content */}
          {/* <div>
